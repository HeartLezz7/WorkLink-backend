const { registerSchema,loginSchema} = require('../validators/auth-admin-validator')
...
exports.register = async (req,res,next) =>{
    try{
      ...
      console.log(`log After Delete Password`,createAdmin)
       ...
       res.status(200).json({ message:"success Register From /admin/register res createAdmin ",createAdmin })
    }catch(err){
        next(err)
    }
}
...
exports.login = async (req,res,next) =>{
    try{
        ...
        const isMatch = await bcrypt.compare(adminLogin.password,findAdminLogin.password)
        ...
        res.status(200).json({ message:"success emailOrPhoneNumber Login From /admin/login", findAdminLogin})
    }catch(err){
        console.log(err)
        next(err)
    }
